...
export const BoxSetting = ({
  ...
  defaultSetting, defaultAmplitudeScaler,
  setShowWindow }) => {

  const [strAmplitudeScaler, setStrAmplitudeScaler] = useState({});
  const [strLambda, setStrLambda] = useState('');
  const timeoutIdRef = useRef();

  useEffect(() => {
    if (!isStateComplete(amplitudeScaler, setting)) return;
    updateStringStates(amplitudeScaler, setting, setStrAmplitudeScaler, setStrLambda);
  }, [amplitudeScaler, setting])
  ...
  const onClick_setToDefault = () => {
    setToDefault(defaultSetting, defaultAmplitudeScaler, setStrLambda, setStrAmplitudeScaler);
    setAmplitudeScaler(defaultAmplitudeScaler);
    setSetting({ ...setting, lambda: defaultSetting.lambda });
  }
  ...
  const handleSetInputTimeout = () => {
    ...
    setStrLambda((currentStrLambda) => {
      const newLambda = isValidNumber(currentStrLambda) ? roundToFourSignificantFigures(currentStrLambda) : setting.lambda;
      ...
      <Front>
        <FrontHeader>
          <FrontHeaderInner>
            <FrontHeaderLeft>
              <TitleWrapper>
                <CustomH3>入力波</CustomH3>
              </TitleWrapper>
              <ButtonSmallWrapper style={{ marginLeft: "30px" }}>
                <ButtonSmall onClick={() => onClick_setToDefault()}>初期設定に戻す</ButtonSmall>
              </ButtonSmallWrapper>
            </FrontHeaderLeft>
          </FrontHeaderInner>
        </FrontHeader>
        ...
        <FrontBody>
          <ColumnLayout>
            <GridColumn>
              {strAmplitudeScaler !== undefined && strAmplitudeScaler.slope !== undefined && strLambda !== undefined && (
                <InputItemGrid>
                  {inputFields.map(({ name, field, type }, index) => {
                    ...
                    const value = (field === 'lambda') ? strLambda : strAmplitudeScaler[field];
                    return (
              ...
              <ButtonReturnWrapper>
                <ButtonReturn onClick={() => setShowWindow("home")}>ホームに戻る</ButtonReturn>
              </ButtonReturnWrapper>
            </GridColumn>
          </ColumnLayout>
        </FrontBody>
