...
export const Home = ({ mediumColor, medium, setShowWindow, setMedium }) => {
  ...
  useEffect(() => {
    if (!medium && !mediumColor && !medium.length >= 2) return;
    const stringDefaultMedium = medium.slice(0, 2).map(item => ({
    ...
    const stringMedium = medium.slice(2).map(item => ({
      "DielectricConstant": item.DielectricConstant.toString(),
      "DielectricLoss": item.DielectricLoss.toString(),
      "MagneticConstant": item.MagneticConstant.toString(),
      "MagneticLoss": item.MagneticLoss.toString()
  ...
  const save = () => {
    ...
    if (error.length === 0) {
      ...
      const firstTwo = medium.slice(0, 2);
